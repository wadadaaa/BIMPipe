...
describe('exportFullIfcWithRisers', () => {
  ...
  it('reports sanitary route export counts and skipped segment reasons in debug mapping', async () => {
    const { api } = makeMockApi()
    const risers: Riser[] = [
      {
        id: 'riser-1',
    ...
    const sanitaryRoutes = [
      {
        ...
        segments: [
          ...
          { from: { x: 1, y: 0, z: 1 }, to: { x: 1, y: 0, z: 1 }, kind: 'branch' as const, pipeDiameterMm: 50 as const },
        ],
      },
      {
        fixtureExpressId: 502,
        fixtureName: 'Missing riser fixture',
        fixtureKind: 'SINK' as const,
        riserId: 'missing-riser',
        pipeDiameterMm: 50 as const,
        startHeightAboveFloorM: 0.15,
        slope: 0.02,
        segments: [
          { from: { x: 2, y: 0, z: 2 }, to: { x: 3, y: 0, z: 3 }, kind: 'branch' as const, pipeDiameterMm: 50 as const },
        ],
      },
    ]
    ...
    const result = await exportFullIfcWithRisersWithDebug(
      ...
      risers,
      null,
      { exportRunId: 'route-debug', timestamp: '2026-06-04T00:00:00.000Z' },
      sanitaryRoutes,
    )

    expect(result.debugMapping.sanitaryRouteExport).toMatchObject({
      receivedRoutes: 2,
      collectedSegments: 2,
      writtenElements: 1,
      skippedSegments: expect.arrayContaining([
        expect.objectContaining({ reason: expect.stringContaining('endpoints coincide in plan') }),
        expect.objectContaining({ reason: expect.stringContaining('references missing riser missing-riser') }),
      ]),
    })
    expect(result.debugMapping.notes).toEqual(
      expect.arrayContaining([
        expect.stringContaining('Received 2 sanitary route(s) for export'),
