...
/**
 * Goal 4 final gate (G6): the five hard metrics of the gauntlet's office floor
 * (second project, MEP storey as office — floor code F2), computed by
 * `computeGauntletMetrics` on the real pipeline output and pinned to the round-3
 * values (`external/gauntlet/rounds/03/F2/metrics.json`, 2026-09-07). Client
 * data lives under the gitignored `external/`; the suite skips cleanly without it.
 *
 * The engineer bar on this floor is partial (his slice serves 24 of the 33
 * detected fixtures; `fixturesOnlyWeServe` = 9 is a documented model gap, not a
 * placement gap). A change that moves any of these numbers is a change to the
 * gauntlet result and must be recorded as such, never re-pinned quietly.
 */
const SPEC = GAUNTLET_BUILTIN_FLOOR_SPECS['shbj-L04']
const TEST_TIMEOUT_MS = 600_000

const PIN_OBSTRUCTION = 0
const PIN_UNKNOWN_PROBES = 0
const PIN_OUR_STACKS = 7
const PIN_ENGINEER_STACKS = 5
const PIN_STACKS_RATIO = 7 / 5
const PIN_MEAN_DIST_TO_ENGINEER_STACK_M = 5.77
const MEAN_DIST_TOLERANCE_M = 0.05
const PIN_BRANCH_RATIO_SHARED = 1.35
const BRANCH_RATIO_TOLERANCE = 0.02
const PIN_ROUTED_FRACTION = 1
const PIN_FIXTURES_ONLY_ENGINEER_SERVES = 0
const PIN_FIXTURES_ONLY_WE_SERVE = 9

const gated = describe.skipIf(!specFilesExist(SPEC))

gated('gauntlet hard metrics on the office floor F2 (gated: requires local client files)', () => {
  it(
    ...
    async () => {
      ...
      try {
        ...
        console.info(`[F2 gauntlet metrics] ${JSON.stringify({ ...metrics, sharedFixtures: undefined })}`)
