...
describe('decideRiserStrategyPerToiletRoom', () => {
  ...
  it('uses deterministic groupId tie-break when candidate and overlap strength metrics are equal', () => {
    const decisions = decideRiserStrategyPerToiletRoom([
      group('g-a', [member('overlap', 101, true), member('a2', 102, true)], 0.9),
      group('g-b', [member('other', 101, true), member('b2', 102, true)], 0.9),
  ...
  it('does not claim self-coverage when group primary is covered by stronger group', () => {
    const decisions = decideRiserStrategyPerToiletRoom([
      group('g-main', [member('shared', 10, true), member('main-upper', 20, true)], 0.95),
      group('g-sub', [member('shared', 10, true), member('sub-upper-unique', 30, true)], 0.7),
    ])

    const subUpper = decisions.find((d) => d.groupId === 'g-sub' && d.areaId === 'sub-upper-unique')
    expect(subUpper).toBeDefined()
    expect(subUpper?.coveredByGroupId).not.toBe('g-sub')
    expect([
      RISER_STRATEGY_DECISION.COVERED_BY_EXISTING_RISER_GROUP,
      RISER_STRATEGY_DECISION.COORDINATION_REQUIRED,
    ]).toContain(subUpper?.decision)
    if (subUpper?.decision === RISER_STRATEGY_DECISION.COVERED_BY_EXISTING_RISER_GROUP) {
      expect(subUpper.coveredByGroupId).toBe('g-main')
      expect(subUpper.reasons).toContain('eligible non-primary member is covered by stronger group serving this group primary area')
    }
  })
