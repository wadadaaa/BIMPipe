...
type PrimaryServingResolution =
  | { mode: 'SELF_PLACED', groupId: string }
  | { mode: 'COVERED_BY_STRONGER', groupId: string }
...
export function decideRiserStrategyPerToiletRoom(
  ...
  for (const group of sortedGroups) {
    const eligibleMembers = sortMembers(group.members, storeyById).filter((member) => member.eligibleForNewRisers)
    const hasEligible = eligibleMembers.length > 0
    const highestEligibleStoreyId = hasEligible ? maxEligibleStoreyId(group, storeyById) : null
    const primaryEligibleAreaId = hasEligible ? eligibleMembers[0].areaId : null
    const primaryServing = resolvePrimaryServing(group, primaryEligibleAreaId, overlappingByArea, profiles)

    for (const member of sortMembers(group.members, storeyById)) {
      ...
      if (!member.eligibleForNewRisers) {
        if (highestEligibleStoreyId !== null && compareStoreysByElevation(member.storeyId, highestEligibleStoreyId, storeyById) > 0) {
          if (primaryServing.mode === 'COORDINATION_REQUIRED') {
            reasons.push('group primary serving is ambiguous; coordination required for penthouse member')
            ...
            decision = createDecision(group, member, RISER_STRATEGY_DECISION.PENTHOUSE_SERVED_BY_EXISTING_RISER, reasons, overlaps, {
              servedByGroupId: primaryServing.groupId,
            })
          }
        } else {
        ...
        if (primaryServing.mode === 'SELF_PLACED') {
          reasons.push('eligible non-primary member is covered by riser placed for this group')
          decision = createDecision(group, member, RISER_STRATEGY_DECISION.COVERED_BY_EXISTING_RISER_GROUP, reasons, overlaps, {
            coveredByGroupId: group.groupId,
          })
        } else if (primaryServing.mode === 'COVERED_BY_STRONGER') {
          reasons.push('eligible non-primary member is covered by stronger group serving this group primary area')
          decision = createDecision(group, member, RISER_STRATEGY_DECISION.COVERED_BY_EXISTING_RISER_GROUP, reasons, overlaps, {
            coveredByGroupId: primaryServing.groupId,
          })
        } else {
          reasons.push('group primary serving is ambiguous; coordination required for non-primary eligible member')
          decision = createDecision(group, member, RISER_STRATEGY_DECISION.COORDINATION_REQUIRED, reasons, overlaps)
        }
      }
...
function resolvePrimaryServing(
...
): PrimaryServingResolution {
  if (!primaryAreaId) return { mode: 'COORDINATION_REQUIRED' }
  const overlaps = (overlappingByArea.get(primaryAreaId) ?? []).filter((group) => group.groupId !== baseGroup.groupId)
  const stronger = overlaps
    .filter((group) => isStrongerGroup(group, baseGroup, profiles))
    .sort((a, b) => compareGroupStrength(a, b, profiles))
  const top = stronger[0]
  const second = stronger[1]
  if (top && second && hasEqualStrength(top, second, profiles)) return { mode: 'COORDINATION_REQUIRED' }
  if (top) return { mode: 'COVERED_BY_STRONGER', groupId: top.groupId }
  return { mode: 'SELF_PLACED', groupId: baseGroup.groupId }
}
